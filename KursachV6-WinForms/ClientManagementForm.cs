...
namespace KursachV6_WinForms
{
    public partial class ClientManagementForm : Form
    {
        ...
        {
            InitializeComponent();
            this.clientManager = clientManager; // Используем переданный объект
            this.Load += ClientManagementForm_Load; // Подключаем событие загрузки
        }
        ...
        private void ClientManagementForm_Load(object sender, EventArgs e)
        {
            ...
            // Настройка столбцов для DataGridView
        ...
        {
            txtSearch.Clear(); // Очищаем текстовое поле поиска
            UpdateClientGrid(); // Обновляем таблицу, отображая всех клиентов
        }


        private void UpdateClientGrid()
        {
            dgvClients.Rows.Clear(); // Очищаем таблицу

            // Получаем список клиентов из ClientManager
