...
namespace KursachV6_WinForms
{
    public partial class ProductManagementForm : Form
    {
        ...
        {
            // Настраиваем столбцы для DataGridView
        ...
        {
            dgvProducts.Rows.Clear(); // Очищаем таблицу

            // Заполняем таблицу данными из ProductManager
        ...
        {
            // Открываем форму добавления товара
            var addProductForm = new AddProductForm(productManager);
            if (addProductForm.ShowDialog() == DialogResult.OK)
            {
                LoadProducts(); // Обновляем таблицу после добавления
            }
        }
        ...
        private void btnEditProduct_Click(object sender, EventArgs e)
        {
            if (dgvProducts.SelectedRows.Count > 0)
            {
                ...
                {
                    LoadProducts(); // Обновляем таблицу после редактирования
                }
            }
            else
            {
        ...
        private void btnDeleteProduct_Click(object sender, EventArgs e)
        {
            if (dgvProducts.SelectedRows.Count > 0)
            {
                ...
                if (confirmResult == DialogResult.Yes)
                {
                    ...
                    LoadProducts(); // Обновляем таблицу после удаления
                }
            }
            else
            {
